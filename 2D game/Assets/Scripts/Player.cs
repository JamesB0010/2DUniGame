...
public class Player : MonoBehaviour
{
    ...
    //when the player stops accelerating a turbo blow off valve sound will play, to stop the sound from becoming repetitive and annoying a minimum time must have passed since the last time the sound has been played, this field stores the time which has passed
    ...
    void Update()
    {
        ...
        //add the forwards direction of the player to the acceleration 
        ...
        Vector3 sidewaysAcceleration = new Vector3(transform.right.x, transform.right.y, transform.right.z) * friction * Time.deltaTime * 2;
        ...
        //this ensures that the big revs sound is only stopped when both players have stopped
        if (FindObjectOfType<Player2>().InputVertical == 0 && inputVertical == 0)
        {
            FindObjectOfType<AudioManager>().Stop("BigRevs");
        }
    }
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        //if the raycast hits something then
        if (hit)
        {
    ...
    private void PunchBoost()
    {
        ...
        int input = (int)Input.GetAxisRaw("Boost1");
        ...
        if (input == 0 || driftGauge <= 0 || minBoostMet == false)
        {
            ...
            //turn the idle sound back up as it was turned to 0 when boosting
            idleSound.volume = 0.6f;
